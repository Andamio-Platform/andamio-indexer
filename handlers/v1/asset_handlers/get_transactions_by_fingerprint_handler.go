...
// GetTransactionsByFingerprintHandler handles the request to get transactions for a specific asset fingerprint.
//
//	@Summary		Get Transactions by Asset Fingerprint
//	@Description	Get transactions associated with a specific asset fingerprint.
//	@ID			getTransactionsByFingerprint
//	@Tags			Assets
//	@Security		ApiKeyAuth
//	@Accept			json
//	@Produce		json
//	@Param			asset_fingerprint	path		string	true	"Asset Fingerprint to get transactions for"
...
func GetTransactionsByFingerprintHandler(c *fiber.Ctx, db *database.Database) error {
	assetFingerprint := c.Params("asset_fingerprint")

	if assetFingerprint == "" {
		fiberLogger.Error("Asset fingerprint is required")
		return fiber.NewError(fiber.StatusBadRequest, "Asset fingerprint is required")
	}
	...
	// Fetch asset ID for the given fingerprint
	...
	if result.Error != nil {
		if result.Error == gorm.ErrRecordNotFound {
			fiberLogger.Errorf("Asset not found for fingerprint %s", assetFingerprint)
			return fiber.NewError(fiber.StatusNotFound, "asset not found")
		} else {
			fiberLogger.Errorf("Failed to fetch asset for fingerprint %s: %v", assetFingerprint, result.Error)
	...
	if result.Error != nil && result.Error != gorm.ErrRecordNotFound {
		fiberLogger.Errorf("Failed to fetch utxo IDs for asset with fingerprint %s: %v", assetFingerprint, result.Error)
	...
	if result.Error != nil && result.Error != gorm.ErrRecordNotFound {
		fiberLogger.Errorf("Failed to fetch paginated transactions for fingerprint %s: %v", assetFingerprint, result.Error)
		return fiber.NewError(fiber.StatusInternalServerError, "failed to fetch transactions")
	}
