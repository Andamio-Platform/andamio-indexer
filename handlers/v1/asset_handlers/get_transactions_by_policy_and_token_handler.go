...
// GetTransactionsByPolicyAndTokenHandler handles the request to get transactions for a specific asset policy ID and token name.
//
//	@Summary		Get Transactions by Policy ID and Token Name
//	@Description	Get transactions associated with a specific asset policy ID and token name.
//	@ID			getTransactionsByPolicyAndToken
//	@Tags			Assets
//	@Security		ApiKeyAuth
//	@Accept			json
//	@Produce		json
//	@Param			policyId	path		string	true	"Asset Policy ID to get transactions for"
...
//	@Failure		404		{object}	errors.ServerError	"Asset not found" // TODO: Use project's error handling
...
func GetTransactionsByPolicyAndTokenHandler(c *fiber.Ctx, db *database.Database) error {
	policyId := c.Params("policyId")
	tokenname := c.Params("tokenname")

	if policyId == "" {
		fiberLogger.Error("Policy ID is required")
	...
	// Fetch asset ID for the given policy ID and token name
	var asset models.Asset
	result := txn.Metadata().Model(&models.Asset{}).
		Where("policy_id = ? AND asset_name = ?", policyId, tokenname).
		First(&asset)

	if result.Error != nil {
		if result.Error == gorm.ErrRecordNotFound {
			fiberLogger.Errorf("Asset not found for policy %s and token %s", policyId, tokenname)
			return fiber.NewError(fiber.StatusNotFound, "asset not found")
		} else {
			fiberLogger.Errorf("Failed to fetch asset for policy %s and token %s: %v", policyId, tokenname, result.Error)
			return fiber.NewError(fiber.StatusInternalServerError, "failed to fetch asset")
		}
	}

	// Fetch utxo IDs associated with this asset ID
	var utxoIDs []uint
	...
	if result.Error != nil && result.Error != gorm.ErrRecordNotFound {
		fiberLogger.Errorf("Failed to fetch utxo IDs for asset with policy %s and token %s: %v", policyId, tokenname, result.Error)
		return fiber.NewError(fiber.StatusInternalServerError, "failed to fetch transactions")
	}

	if len(utxoIDs) == 0 {
		return c.JSON([]models.Transaction{}) // No utxos found for this asset
	}
	...
	if result.Error != nil && result.Error != gorm.ErrRecordNotFound {
		fiberLogger.Errorf("Failed to fetch paginated transactions for policy %s and token %s: %v", policyId, tokenname, result.Error)
		return fiber.NewError(fiber.StatusInternalServerError, "failed to fetch transactions")
	}
