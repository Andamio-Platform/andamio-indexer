...
// GetTransactionsByPolicyHandler handles the request to get transactions for a specific asset policy ID.
//
//	@Summary		Get Transactions by Policy ID
//	@Description	Get transactions associated with a specific asset policy ID.
//	@ID			getTransactionsByPolicy
//	@Tags			Assets
//	@Security		ApiKeyAuth
//	@Accept			json
//	@Produce		json
...
func GetTransactionsByPolicyHandler(c *fiber.Ctx, db *database.Database) error {
	...
	// Fetch asset IDs for the given policy ID
	var assetIDs []uint
	...
	if result.Error != nil && result.Error != gorm.ErrRecordNotFound {
		fiberLogger.Errorf("Failed to fetch asset IDs for policy %s: %v", policyId, result.Error)
		return fiber.NewError(fiber.StatusInternalServerError, "failed to fetch transactions")
	}

	if len(assetIDs) == 0 {
		return c.JSON([]models.Transaction{}) // No assets found for this policy
	}
	...
	if result.Error != nil && result.Error != gorm.ErrRecordNotFound {
		fiberLogger.Errorf("Failed to fetch utxo IDs for assets with policy %s: %v", policyId, result.Error)
		return fiber.NewError(fiber.StatusInternalServerError, "failed to fetch transactions")
	}
	...
	if result.Error != nil && result.Error != gorm.ErrRecordNotFound {
		fiberLogger.Errorf("Failed to fetch paginated transactions for policy %s: %v", policyId, result.Error)
		return fiber.NewError(fiber.StatusInternalServerError, "failed to fetch transactions")
	}
